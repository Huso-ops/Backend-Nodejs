...
exports.addPostComment = (data, callback) => {
  db.query(
    "INSERT INTO posts(postId,Comment,datetimeCreated, addedByUserId)VALUES(?,?,?,?)",
    [data.description, data.imagePath, new Date(), data.addedByUserId],
    (error, results, fields) => {
      if (error) {
        return callback(error);
...
exports.getPostAllComment = (data, callback) => {
  db.query(
...
exports.deleteePost = (data, callback) => {
  db.query(
    `DELETE FROM posts
    WHERE 
    id = ?`,
    ...
    (error, results, fields) => {
      ...
      if (results.affectedRows === 1) {
        return callback(null, `Deleted Successful`);
      } else {
        return callback(new Error("Invalid post"));
      }
    }
